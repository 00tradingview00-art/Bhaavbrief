<!--
prompts/brief_v4.md — Part 8.2 versioned system prompt for the daily brief.

v4 (2026-07-29): added a guardrail against chaining 3+ historical price
references into one compounding arithmetic narrative — e.g. "three sessions
ago crude was ₹9,024 → two sessions of unwinding took it to ₹7,841 →
today's ₹X48 move," where the implied total contradicts today's actual
snapshot price. This is a distinct defect from the v2/v3 fixes (stale-price-
as-prevClose, manual arithmetic): each historical reference here was
individually plausible, but chaining them together produced a narrative
that didn't arithmetically close. Recurred 3 times on 2026-07-29 alone.

v2 (2026-07-27): added a guardrail against treating a price mentioned in
{{HISTORY_BLOCK}} prose as an authoritative "last session" figure — the
model was computing deltas against stale prices quoted in prior editions'
narrative text instead of the snapshot's own prevClose, tripping the
publish gate's SEMANTIC-BLOCK check twice in a row (edition #72 attempts).

v3 (2026-07-27): stopped asking the model to subtract price - prevClose
itself — it kept getting the arithmetic wrong (e.g. stated a ₹838 crude
drop when the actual figure was ₹418), tripping SEMANTIC-BLOCK a third
time even after v2 fixed the stale-price-source bug. generate-brief.js's
PRICE_BLOCK now includes every absolute ₹/$ change pre-computed in JS
(always correct); this version instructs the model to use those figures
verbatim instead of calculating deltas manually.
...
- RECENT EDITIONS (the {{HISTORY_BLOCK}} above) are for narrative/story continuity only — NEVER treat a price mentioned in that prior-edition prose as "yesterday's close" or use it in a delta calculation. Every price comparison (today vs. last session) must be computed only from the prevClose values in the price data above.
- NEVER calculate a price change, gap, or "from X to Y" arithmetic yourself. The price data above already gives you the exact ₹/$ change from last close for every instrument ("change ₹..." / "change $..."). Always quote that pre-calculated figure verbatim — do not subtract two prices to derive your own number, even if it looks like simple arithmetic. If you need a change figure that isn't provided, describe the move in words instead of stating a number.
- NEVER chain more than one historical reference point into a single arithmetic narrative (e.g. "X three sessions ago → Y two sessions ago → Z today" implying the moves add up to a third figure). Each historical comparison must stand alone against exactly ONE reference — either the prevClose value in the price data above, or one specifically named prior edition — never implied to sum or compound into another number. If you want an older price point for narrative color, state it as an isolated fact ("the story began near ₹X") without deriving today's change from it.
