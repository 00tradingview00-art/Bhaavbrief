...
export default function BriefPage({ params }: { params: { slug: string } }) {
  ...
  // JSON-LD: NewsArticle schema — huge for Google News and rich results
  const articleSchema = {
  ...
  return (
    <div style={{ background: '#FAFAF6', minHeight: '100vh' }}>
      ...
      {/* Breadcrumb — visible nav for SEO and UX */}
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '1.5rem 1.5rem 3rem', display: 'grid', gridTemplateColumns: '1fr 280px', gap: '3rem' }}>
        ...
        <main>
          <article itemScope itemType="https://schema.org/NewsArticle">
            ...
            <header style={{ paddingBottom: '1.5rem', marginBottom: '1.5rem', borderBottom: '0.5px solid #DDDDD0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '0.75rem', flexWrap: 'wrap' }}>
                <span style={{
                  ...
                  border: `0.5px solid ${tagStyle.borderColor}`,
                  background: tagStyle.background as string,
                  color: tagStyle.color as string,
                }}>
                  {brief.tags?.[0] ?? 'Brief'}
                </span>
                <time
                  ...
                  style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: '#8A8A7A', letterSpacing: '0.05em' }}
                >
              ...
              {brief.summary && (
                <p
                  ...
                  style={{
                    fontSize: '1rem', color: '#48483A', lineHeight: 1.7,
                    fontWeight: 300, paddingLeft: '1rem',
                    borderLeft: '0.5px solid #C8C8B8', margin: 0,
                  }}
                >
                  {brief.summary}
                </p>
              ...
              {/* Commodity tags — keyword-rich anchors */}
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: '0.75rem' }}>
                  {brief.commodities.map(c => (
                    <span
                      key={c}
                      itemProp="keywords"
                      style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 9, color: '#8A8A7A', background: '#F3F2EC', padding: '2px 8px' }}
                    >
                      {c}
                    </span>
                  ))}
            ...
            <div style={{ marginTop: '2rem', padding: '1rem', background: '#F3F2EC', border: '0.5px solid #DDDDD0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
              <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, letterSpacing: '0.04em', color: '#48483A' }}>
                Found this useful? Share it with your trading circle.
              </span>
              <div style={{ display: 'flex', gap: 8 }}>
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(brief.title)}&url=${encodeURIComponent(url)}&via=bhaavbrief`}
                  target="_blank" rel="noopener noreferrer"
                  style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, letterSpacing: '0.04em', color: '#FAFAF6', background: '#18180F', padding: '6px 14px', textDecoration: 'none' }}
                >
                  Share on X
                </a>
                <a
                  href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`${brief.title} — ${url}`)}`}
                  target="_blank" rel="noopener noreferrer"
                  style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, letterSpacing: '0.04em', color: '#fff', background: '#25D366', padding: '6px 14px', textDecoration: 'none' }}
                >
                  Share on WhatsApp
                </a>
              </div>
        ...
        <aside style={{ borderLeft: '0.5px solid #DDDDD0', paddingLeft: '2rem', alignSelf: 'start' }}>
          ...
          <div style={{ background: '#18180F', padding: '1.25rem', color: '#FAFAF6' }}>
            ...
            <p style={{ fontSize: 11, color: 'rgba(250,250,246,0.55)', lineHeight: 1.6, fontWeight: 300, margin: 0 }}>
              BhaavBrief is for informational and educational purposes only. This is not investment advice. Always conduct your own research before making any trading or investment decisions.
            </p>
          </div>
        </aside>
      </div>
