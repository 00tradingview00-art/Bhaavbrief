...
export default function MarketPulsePanel({ terminalData, prices }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{ background: 'var(--surface)', padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ fontFamily: 'var(--font-sans)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-3)' }}>
          MCX Composite iVIX
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 30, fontWeight: 600, color: 'var(--ink)' }}>
      ...
      <div style={{ background: 'var(--surface)', padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ fontSize: 11.5, color: usdinr > 0 ? (usdinrUp ? 'var(--up)' : 'var(--down)') : 'var(--ink-3)' }}>
          {usdinr > 0 ? (
            <>
