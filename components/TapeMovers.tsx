...
function TapeMoversRow({ event, commodityKey }: { event: EventMapEntry; commodityKey: string }) {
  ...
  return (
    <div className="tape-movers-row" style={{ padding: '10px 0', borderBottom: '0.5px solid #DDDDD0' }}>
      <div>
        <div style={{ fontFamily: 'var(--font-sans)', fontSize: 12, fontWeight: 600, color: '#0E0D0A' }}>
          {event.name}
        </div>
        <div style={{ fontFamily: 'var(--font-sans)', fontSize: 10, color: '#8A8A7A', marginTop: 2 }}>
  ...
  return (
    <div className="tape-movers-widget" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <span style={{ fontFamily: 'var(--font-sans)', fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A8A7A' }}>
          Today&rsquo;s Tape Movers
        </span>
        <Link href="/calendar" style={{ fontFamily: 'var(--font-sans)', fontSize: 10, color: '#C8720A', textDecoration: 'none' }}>
          Full calendar →
        </Link>
      </div>
