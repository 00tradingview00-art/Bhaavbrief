...
/**
 * WTI crude USD/barrel + USD/INR -> INR per barrel import parity.
 * MCX crude is quoted INR/barrel (direct unit — no lot-size conversion needed here).
 * The spread (MCX minus parity) captures: refinery premium/discount, customs duty,
 * and any domestic supply-demand divergence from the global benchmark.
 */
export function computeImportParityCrudeINR(wtiBarrelUSD, usdinr) {
  if (!(wtiBarrelUSD > 0) || !(usdinr > 0)) return 0
  return Math.round(wtiBarrelUSD * usdinr)
}

/**
 * COMEX HG copper USD/cent-per-lb + USD/INR -> INR per kg import parity.
 * MCX copper is quoted INR/kg. COMEX HG=F is cents/lb.
 * Conversion: cents/lb ÷ 100 = USD/lb; × 2.20462 = USD/kg; × USDINR = INR/kg.
 * Note: uses COMEX HG=F as LME proxy (true LME requires a paid data feed).
 */
export function computeImportParityCopperINR(comexCentPerLb, usdinr) {
  if (!(comexCentPerLb > 0) || !(usdinr > 0)) return 0
  return Math.round((comexCentPerLb / 100) * 2.20462 * usdinr)
}
