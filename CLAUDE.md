...
Do not let gold dominate or treat untested lanes as failures. Use the actual metric
coverage in `scripts/analyze-reel-history.mjs`; missing saves/shares are not zero.
V3 is the only permitted publishing format and uses the female Edge voice; preserve
existing posts/history. Legacy Reel workflows and uploads are retired. Do not
redefine the strategy from a tiny pilot sample. Keep these facts synchronized with
`AGENTS.md`.
